...
const getTires = async (querys = '') => {
  const origin = window.location.origin,
    ...
    apiSearch = `${tires}${
      querys || '?'
    }&inventario=Con%20renovador&size=${size}&page=${page}`;
  ...
  try {
    ...
    result.forEach((item) => {
      ...
      template.querySelector('.add-item').style.display = 'none';
      ...
      if (params.get('renovador')) {
        template.querySelector('.add-item').style.display = 'flex';
      }
